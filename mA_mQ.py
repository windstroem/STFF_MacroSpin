...
m0Q   = np.array([ma.cos(0.1)*ma.sin(1.8),ma.sin(0.1)*ma.sin(1.8),ma.cos(1.8)])
mAQ0  = np.hstack((m0,m0))
#print(mAQ0)
...
def f(t,m,prefactor,Ms,N,K1vec,K1,alpha,gamma,V,Temp,dt,p,s,epsilon,I):
 ...
 heffA      = Heff(mA,Ms,N,K1vec,K1,alpha,gamma,V,Temp,dt)
 precesionA = np.cross(mA,heffA)
 dampingA   = alpha*np.cross(mA,precesionA)
 sttA       = tau(mA,s,I,p,V,Ms,gamma,epsilon,gox(p,mA,s))
 rhsA       = precesionA + dampingA + sttA
 heffQ      = Heff(mQ,Ms,N,K1vec,K1,alpha,gamma,V,Temp,dt)
 precesionQ = np.cross(mQ,heffQ)
 dampingQ   = alpha*np.cross(mQ,precesionQ)
 sttQ       = tau(mQ,s,I,p,V,Ms,gamma,epsilon,gox(p,mQ,s))
